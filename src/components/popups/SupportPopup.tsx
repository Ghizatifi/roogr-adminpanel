...
const SupportPopup = ({
  ...
  const handleCategorySubmit = async (
    ...
    try {
      setSubmitting(true);
      const res = await axiosInstance.put(`/support/${ticket_id}`, values, {
                    ...
                    <Form>
                      <div className="m-auto w-[90%]">
                        <h3 className="mb-2 font-semibold dark:text-white">
                          {t('contact-us.ticketDetails')}
                        </h3>
                        <div className="m-auto w-[90%] dark:bg-white dark:text-header-inputDark p-2 rounded-md">
                          {enquiry}
                        </div>
                      </div>
                      <label
